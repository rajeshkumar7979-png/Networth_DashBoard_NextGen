...
- **Provider abstraction:** an `AIProviderClient` Protocol + `register_ai_provider/get_ai_client/build_client` registry. Default transport is local Ollama (keyless `ollama_local`, `llama3.1:8b` at `http://localhost:11434/v1`, OpenAI-compatible `/chat/completions`; `openai/gpt-oss-120b` selectable via `AI_MODEL`) with `json_schema` Structured Outputs attempted first and a single 400-degrade retry to `json_object`. Groq's live default is `openai/gpt-oss-20b` (`json_object`; `json_schema` is accepted on gpt-oss-20b / gpt-oss-120b / qwen/qwen3.8-27b). Retired Llama ids are remapped. The prompt spells out every field's exact type so parsing still recovers the schema. The local parser (`parse.py`) recovers the two scalar string fields a drifting model most often mangles (`overall_assessment`, `uncertainty` — object/list/bool/null → bounded plain text with a labeled downgrade) instead of failing the whole answer; genuinely broken shapes still classify `malformed`. The raw model response is logged at DEBUG (bounded 1500-char excerpt, redacted, never shown in the UI) before validation. A provider 404/'model discontinued' rejection raises a clear `AI model {model} is no longer supported. Please update AI_MODEL in your secrets file.` error (see `lib/intelligence/ai/client.py` — no degrade retry is wasted on a discontinued model).
- **Cascade (graceful degradation):** on a retriable network error (timeout/connection refused) with **no explicit `client=` override**, `run_ai_research` falls back Groq → **Ollama local** (`ollama_local`, `http://localhost:11434/v1`, `llama3.1:8b`, keyless, 180 s timeout) → "AI unavailable". The Command Center's AI expander header shows the active provider (Groq / Ollama local / Unavailable / Not configured).
- **Ollama local health check & first-run UX (`lib/intelligence/ai/health.py`):** the inference timeout default is **180 s** (`DEFAULT_TIMEOUT_SECONDS` / `OLLAMA_TIMEOUT_SECONDS` in `config.py`), because a cold local model can take minutes to load. `check_ollama_health()` probes `http://localhost:11434/api/tags` (short 3 s liveness timeout) and returns a plain dict (`ok`, `error`, `models`, `count`, `latency_ms`) — it never raises and never fabricates a healthy result. The Command Center only probes on explicit user actions (the `Check Ollama connection` button and the `Run AI research` button), so **ordinary page loads stay network-free**. When the probe fails the page shows `OLLAMA_NOT_RUNNING_HINT` ("Ollama is not running. Start it with: ollama serve") and, when `ollama` is on PATH (`ollama_command_available()`), a `Start Ollama Service` button that calls `start_ollama_service()` to spawn a detached `ollama serve` background process (Windows `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP`; controlled failure when the executable is missing). A failed local run is given targeted guidance via `ollama_failure_hint()`: timeout → `OLLAMA_TIMEOUT_HINT` (wait 2-3 min / smaller `llama3.2:3b` / 8GB+ RAM), connection failure → the not-running hint. The run button's spinner uses `OLLAMA_FIRST_RUN_SPINNER` ("AI is thinking... (this may take 2-3 minutes on first run)").
- Pinned by `tests/test_ai_health.py` (fully offline — health probes, spawn, and hint selection are mocked; no real server, no key, no network).
- **Config (Streamlit secrets override env; no hard-coded secrets):** the primary source for `AI_API_KEY` is Streamlit's native secrets (`st.secrets["ai"]["AI_API_KEY"]`, held in the git-ignored `.streamlit/secrets.toml`); everything falls back to the environment keys `AI_API_KEY`, `AI_PROVIDER`, `AI_BASE_URL`, `AI_MODEL`, `AI_TIMEOUT_SECONDS`, `AI_MAX_TOKENS`, `AI_TEMPERATURE`, `AI_STRUCTURED_OUTPUT`, `AI_MAX_EVIDENCE_CATALOG` (default 20). When `AI_API_KEY` is set and `AI_PROVIDER` is omitted, the provider is Groq (`openai/gpt-oss-20b` at `api.groq.com`, 60 s) so Streamlit Cloud does not target localhost Ollama; explicit `AI_PROVIDER=ollama_local` still wins. Retired Groq Llama ids (`llama-3.3-70b-versatile` and earlier) are remapped to `openai/gpt-oss-20b`. `ollama_local` is a **keyless** provider — `provider_is_configured()` marks it configured without any key. Secrets are read only when Streamlit's runtime exists (`st.runtime.exists()`), so tests/scripts never touch them. `ai_config_status()` reports configuration **without ever exposing the key**; `redact()` is the single scrub choke point for error strings.
