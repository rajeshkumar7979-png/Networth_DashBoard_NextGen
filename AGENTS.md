...
- **Provider abstraction:** an `AIProviderClient` Protocol + `register_ai_provider/get_ai_client/build_client` registry. Default transport is Groq (`openai/gpt-oss-120b`, OpenAI-compatible `/chat/completions`) with strict `json_schema` Structured Outputs and a single 400-degrade retry to `json_object` for models that reject the JSON schema.
- **Config (env only, no hard-coded secrets):** `AI_API_KEY`, `AI_PROVIDER`, `AI_BASE_URL`, `AI_MODEL`, `AI_TIMEOUT_SECONDS`, `AI_MAX_TOKENS`, `AI_TEMPERATURE`, `AI_STRUCTURED_OUTPUT`, `AI_MAX_EVIDENCE_CATALOG` (default 12). `ai_config_status()` reports configuration **without ever exposing the key**; `redact()` is the single scrub choke point for error strings.
...
- **Entry points:** `build_live_plan(stock_symbols=, fund_names=, gold_symbols=, has_usd_book=, now=)` (planner: word-boundary query derivation — up to 6 stock queries, 3 fund queries, generic gold queries "Sovereign Gold Bond"/"Gold ETF India" plus per-ETF symbol queries except `SGB*`; FRED USD/INR target `DEXINUS` **only** when a USD book is present), `load_live_cohort(...)` (network-free cache read, used on page load), `run_live_research(...)` (the ONLY network entry point; explicit button), `live_status(...)` (cache-scan, flags corrupt/stale entries), `development_rows(cohort, index, now)` (UI rows; `affected_for`/`assess_relevance` use **exact identifier matching only** via `sources/mapping.py`).
